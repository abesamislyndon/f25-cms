...
class Item_model extends CI_Model
   ...
   public function do_add_update_item($item_quantity, $id, $company_name,$item_quantity1, $item_date, $invoice_no, $item_category)
    ...
    public function do_update_item_info($id, $item_category)
    {
      ...
      public function do_delete_item($id, $item_category)
       ...
       if($item_category == 'Small Machines')
           {
            $cat = 'SmallMachines';
           }
          elseif($item_category == 'Metal Working Machines')
           {
              $cat = 'MetalWorkingMachines';
           }
          elseif($item_category == 'Welding Equipments')
           {
              $cat = 'WeldingEquip';
           }
           elseif($item_category == 'Wood Working Machines')
           {
             $cat = 'WoodWorkingMachines';
           }
           elseif($item_category == 'Hardware & Tooling Supplies')
           {
             $cat = 'HardwareToolSupplies';
           }
           elseif($item_category == 'Material Handling Equipment')
           {
             $cat = 'MaterialEquip';
           }
           elseif($item_category == 'Air Compressor')
           {
             $cat = 'AirCompressor';
           }
           elseif($item_category == 'Construction Equipments')
           {
             $cat = 'ConstructionEquip';
           }

        $this->db->where('id',$id);
        $this->db->delete('item');
         
        $this->session->set_flashdata('msg', 'SUCCESFULLY delete');http://www.google.com.sg/aclk?sa=l&ai=C9Wsq6_C9VMrqJcLg8QXR24LoCaGh0MQDufOLx9ABuuPbrWwIABABYL8FoAGchJL_A8gBAaoEJk_QxmkepZZhs4D1dbnuY4avhdGa3S2r7P18YfuyHrH9em6RYZsUgAfM-22IBwGQBwKoB6a-Gw&sig=AOD64_1ZnOdC2JGlWNjEHeXzPNkeB9GqCw&rct=j&q=&ved=0CBwQ0Qw&adurl=http://www.fusioncharts.com/javascript-chart-fiddles/
        redirect('main/inventory/'.$cat); 
       }

      public function retrieve_products(){
            $query = $this->db->get('item_cart'); // Select the table products
